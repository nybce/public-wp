...
if (!class_exists('\Wpo\SCIM\SCIM_Users')) {
    ...
    class SCIM_Users
    {
        ...
        public static function create_user($scim_usr)
        {
            ...
            if (false !== WordPress_Helpers::stripos($wpo_usr->upn, '#ext#')) {
        ...
        public static function update_user($id, $scim_ops)
        {
            ...
            foreach ($scim_ops as $operation) {
                ...
                // userName
                if (WordPress_Helpers::stripos($operation['path'], 'userName') !== false) {
                    Log_Service::write_log('WARN', __METHOD__ . ' -> Updating the user name for user ' . $wp_usr->user_login . ' is not supported');
                    continue;
                }

                // email
                if (WordPress_Helpers::stripos($operation['path'], 'emails') !== false && WordPress_Helpers::stripos($operation['path'], 'work') !== false) {
                ...
                if (WordPress_Helpers::stripos($operation['path'], 'name.') !== false) {

                    if (WordPress_Helpers::stripos($operation['path'], 'name.givenName') !== false) {
                        $wp_usr_key = 'first_name';
                    } elseif (WordPress_Helpers::stripos($operation['path'], 'name.familyName') !== false) {
                        $wp_usr_key = 'last_name';
                    } elseif (WordPress_Helpers::stripos($operation['path'], 'name.formatted') !== false) {
        ...
        private static function try_get_property($resource, $property)
        {
            ...
            if (!is_array($matches) || count($matches) !== 2) {
                ...
                if (WordPress_Helpers::stripos($property, 'urn:ietf:params:scim:schemas:extension:enterprise:2.0:User') === false) {
        ...
        private static function try_add_property(&$resource, $property, $value)
        {
            ...
            if (!is_array($matches) || count($matches) !== 2) {
                ...
                if (WordPress_Helpers::stripos($property, 'urn:ietf:params:scim:schemas:extension:enterprise:2.0') == -1) {
                    $complex_property = explode('.', $property);
