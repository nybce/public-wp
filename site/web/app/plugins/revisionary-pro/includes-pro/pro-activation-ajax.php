...
require_once(RVY_ABSPATH . '/includes-pro/pro-maint.php');
...
switch ($_GET['rvy_ajax_settings']) {
    case 'activate_key':
        ...
        $request_vars = [
            ...
            'license' => pp_revisions_sanitize_key($_GET['key']),
            'url' => site_url(''),
        ];
        ...
        if (is_object($result) && ('valid' == $result->license)) {
            $setting = ['license_status' => $result->license, 'license_key' => pp_revisions_sanitize_key($_GET['key']), 'expire_date' => $result->expires];
