...
if (strpos($_SERVER['REQUEST_URI'], 'p-admin/revision.php') || (strpos($_SERVER['REQUEST_URI'], 'p-admin/admin-ajax.php') && ('get-revision-diffs' == $_REQUEST['action']))) {
	...
	//remove_action( 'wpml_loaded', 'wpml_loaded' );
}
...
class RevisionaryWPML {
	public static function actParseQuery(WP_Query $query) {
		if (!is_admin() && !empty($_REQUEST['preview']) && (!defined('REST_REQUEST') || ! REST_REQUEST) && (!defined('DOING_AJAX') || ! DOING_AJAX)) {
	...
	public static function flt_wpml_posts_request($query) {
		...
		//if ( strpos($query, "FROM $wpdb->posts") && ( strpos($query, ".post_status = 'pending-revision'") || strpos($query, ".post_status = 'future-revision'") || empty($_GET['post_status']) || strpos( $query, "GROUP by post_status" ) ) ) {
		if ( strpos($query, "FROM $wpdb->posts") && ( strpos($query, "'draft-revision'") || strpos($query, "'pending-revision'") || strpos($query, "'future-revision'") ) ) {
			...
			if ( strpos($query, 'ELECT') ) {
				...
				if ( preg_match(
					...
					$post_type = (!empty($_REQUEST['post_type'])) ? 'post_' . pp_revisions_sanitize_key($_REQUEST['post_type']) : 'post_post';
					...
					$parent_where = '(';
					foreach ( $wpml_post_types as $type ) {
						$type = pp_revisions_sanitize_key($type);
