<?php
/**
 * Search Hero
 *
 * @package NYBC
 */

if ( ! defined( 'ABSPATH' ) ) {
	exit;
}

?>
<div class="section banner">

	<div class="banner-inner type-2">

		<div class="banner-wrapper">

			<div class="container container-lg">
				<div class="row justify-content-center">
					<div class="col-lg-10">
						<div class="banner-content">

							<div class="h5 title fw-800 light"><?php esc_html_e( 'Search Results', 'nybc' ); ?></div>

							<div class="spacer-24"></div>

							<h1 class="h1 title fw-800 light">“<?php echo esc_html( get_search_query() ); ?>”</h1>

							<div class="spacer-24"></div>

							<form class="btn-search type-2" action="<?php echo esc_url( NYBC_HOME_URI ); ?>">
								<?php wp_nonce_field( 'search', 'nonce' ); ?>
								<input name="s" class="input" type="search" placeholder="<?php esc_html_e( 'Search for Something Else', 'nybc' ); ?>">
								<button type="submit">
									<svg width="20" height="20" viewBox="0 0 20 20" fill="none"
										xmlns="http://www.w3.org/2000/svg">
										<path
											d="M7.5 13.334a5.8 5.8 0 003.489-1.167l5.677 5.679 1.179-1.179-5.678-5.678A5.8 5.8 0 0013.333 7.5 5.833 5.833 0 107.5 13.334zm0-10A4.171 4.171 0 0111.666 7.5 4.171 4.171 0 017.5 11.667 4.171 4.171 0 013.333 7.5 4.171 4.171 0 017.5 3.334z"
											fill="#FC1921" />
									</svg>
								</button>
							</form>

						</div>
					</div>
				</div>
			</div>
		</div>

		<div class="bg orange"></div>

	</div>

	<div class="spacer-48 spacer-xs-48"></div>

</div>
