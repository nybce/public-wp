...
jQuery(function ($) {
  ...
  $(function () {
    for (let i = 0; i < 12; i++) {
      ...
      $('.months').append(
        '<a class="month" tabindex="0" data-month=' +
          ('0' + (i + 1)).slice(-2) +
          '>' +
          month +
          '</a>'
  ...
  function switchSelectedYear(thisObj) {
    const oldContent = thisObj
      .parent()
      .parent()
      .find('.calendar .input-calendar p')
      .text();
    const newContent =
      oldContent.slice(0, 3) +
      thisObj.parent().parent().find('.date_selector .year-selected').val();
    thisObj
      .parent()
      .parent()
      .find('.calendar .input-calendar p')
  ...
  $('.input-calendar').click(function () {
    ...
    if ($(this).parent().hasClass('date_pick')) {
      open =
        $('.date_selector.active').length > 0 &&
        $(this).parent().find('.date_selector').length > 0 &&
        $('.date_selector.active')[0] ===
          $(this).parent().find('.date_selector')[0];
    } else if ($(this).parent().hasClass('select_box')) {
      open =
        $('.box_selector.active').length > 0 &&
        $(this).parent().find('.box_selector').length > 0 &&
        $('.box_selector.active')[0] ===
          $(this).parent().find('.box_selector')[0];
    }
  ...
  $(document).on('click', function (e) {
    if (
      $(e.target).is(
        '.date_selector.active a, .input-calendar, .input-calendar p, .calendar .input-calendar img, .date_selector.active .picker-select-arrow, .years, .years .year-selected, .date_selector.active div.months, .picker-arrow'
      ) === false
    ) {
      $('.date_selector.active').removeClass('active');
      $('.box_selector.active').removeClass('active');
  ...
  window.picker_date = function () {
    ...
    $('.picker-left').click(function () {
      const value =
        parseInt($(this).parent().parent().find('.year-selected').val()) - 1;
  ...
  $('.months').on('click', '.month', function () {
    ...
    $(this)
      ...
      .parent()
      .parent()
      .find('.input-calendar p')
      .text(month + '/' + year);
    $(this)
      .parent()
      .parent()
      .find('.years .year-selected')
      .val(month + '/' + year);
  });
  ...
  $('.input-calendar, .picker-left, .picker-right').keypress(function (event) {
    const keycode = event.keyCode ? event.keyCode : event.which;
    if (keycode === 13) {
      event.preventDefault();
      $(this).click();
  ...
  $('.months').on('keypress', '.month', function (event) {
    const keycode = event.keyCode ? event.keyCode : event.which;
    if (keycode === 13) {
      event.preventDefault();
      $(this).click();
